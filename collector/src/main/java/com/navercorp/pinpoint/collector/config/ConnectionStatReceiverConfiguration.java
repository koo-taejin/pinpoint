 ...
 * @author Taejin Koo
 */
public class ConnectionStatReceiverConfiguration {

    private final Logger logger = LoggerFactory.getLogger(getClass());

    private static final String GRPC_PREFIX = "collector.receiver.grpc.connection.stat";

    @Value("${collector.receiver.grpc.connection.stat.enable:true}")
    private boolean isGrpcEnable;
    @Value("${collector.receiver.grpc.connection.stat.ip:0.0.0.0}")
    private String grpcBindIp;
    @Value("${collector.receiver.grpc.connection.stat.port:39994}")
    private int grpcBindPort;

    @Value("${collector.receiver.grpc.connection.stat.server.executor.thread.size:128}")
    private int grpcServerExecutorThreadSize;
    @Value("${collector.receiver.grpc.connection.stat.server.executor.queue.size:5120}")
    private int grpcServerExecutorQueueSize;
    @Value("${collector.receiver.grpc.connection.stat.server.executor.monitor.enable:false}")
    private boolean grpcServerExecutorMonitorEnable;

    @Value("${collector.receiver.grpc.connection.stat.worker.executor.thread.size:128}")
    private int grpcWorkerExecutorThreadSize;
    @Value("${collector.receiver.grpc.connection.stat.worker.executor.queue.size:5120}")
    private int grpcWorkerExecutorQueueSize;
    @Value("${collector.receiver.grpc.connection.stat.worker.executor.monitor.enable:false}")
    private boolean grpcWorkerExecutorMonitorEnable;

    @Value("${collector.receiver.grpc.connection.stat.stream.scheduler.thread.size:1}")
    private int grpcStreamSchedulerThreadSize;
    @Value("${collector.receiver.grpc.connection.stat.stream.call.init.request.count:1000}")
    private int grpcStreamCallInitRequestCount;
    @Value("${collector.receiver.grpc.connection.stat.stream.scheduler.period.millis:64}")
    private int grpcStreamSchedulerPeriodMillis;
    @Value("${collector.receiver.grpc.connection.stat.stream.scheduler.recovery.message.count:10}")
    private int grpcStreamSchedulerRecoveryMessageCount;

    @Value("${collector.receiver.grpc.connection.stat.stream.idletimeout:-1}")
    private long grpcStreamIdleTimeout;

    private ServerOption grpcServerOption;


    public ConnectionStatReceiverConfiguration(Environment environment) {
    ...
    public String toString() {
        final StringBuilder sb = new StringBuilder("ConnectionStatReceiverConfiguration{");
