 ...
 */
@Component
public class HbaseConnectionCalleeDataMapper implements RowMapper<ConnectionStatVo> {
    ...
    public ConnectionStatVo mapRow(Result result, int rowNum) throws Exception {
        ...
        if (inetFamily == null) {
            logger.warn("Invalid connectionFamily.");
            throw new IllegalArgumentException("Invalid connectionFamily.");
        }

        final byte[] srcAddress = Arrays.copyOfRange(rowKey, offset.get(), offset.addAndGet(inetFamily.getAddressSize()));

        ConnectionStatVo.Builder builder = new ConnectionStatVo.Builder();
        builder.inetFamily(inetFamily);
        builder.srcIp(srcAddress);
        builder.direction(Direction.OUTGOING);
        ...
        for (Cell cell : rawCells) {
            if (CellUtil.matchingFamily(cell, ConnectionMapHbaseColumnFamily.CONNECTION_STAT_CALLEE.getName())) {
                ...
                Offset dstAddressOffset = new Offset(qualifierBuffer.getOffset());
                final byte[] dstAddress = Arrays.copyOfRange(qualifierArray, dstAddressOffset.get(), dstAddressOffset.addAndGet(InetFamily.INET_4_ADDRESS.getAddressSize()));
                builder.dstIp(dstAddress);

                qualifierBuffer.setOffset(dstAddressOffset.get());
