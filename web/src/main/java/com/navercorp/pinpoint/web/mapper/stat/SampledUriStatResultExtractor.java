...
public class SampledUriStatResultExtractor implements ResultsExtractor<List<SampledAgentUriStat>> {
    ...
    private Map<String, List<EachUriStatBo>> divideByUri(ResultScanner results) throws Exception {
        ...
        for (Result result : results) {
            for (AgentUriStatBo dataPoint : this.rowMapper.mapRow(result, rowNum++)) {
                List<EachUriStatBo> eachUriStatBoList = dataPoint.getEachUriStatBoList();
                if (CollectionUtils.nullSafeSize(eachUriStatBoList) == 0) {
                    continue;
                }
                ...
                for (EachUriStatBo eachUriStatBo : eachUriStatBoList) {
                    ...
                    eachUriStatBo.setAgentId(dataPoint.getAgentId());
                    eachUriStatBo.setStartTimestamp(dataPoint.getStartTimestamp());
                    eachUriStatBo.setTimestamp(dataPoint.getTimestamp());
    ...
    private SampledAgentUriStat getSampleData(List<EachUriStatBo> eachUriStatBos) {
        ...
        SampledAgentUriStat sampledAgentUri = new SampledAgentUriStat(sampledDataPoints);
        return sampledAgentUri;
    }
